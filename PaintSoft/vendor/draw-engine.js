...
// Bresenham's line algorithm: walks integer pixel coordinates from (x0,y0) to (x1,y1),
// advancing by exactly one pixel per iteration along whichever axis dominates. Used for the
// fully-hard, ~1px-wide case below instead of sampling the segment at fixed arc-length
// intervals and rounding each sample's x/y independently — that approach can round two
// nearby samples to the same column but different rows (or vice versa), painting an
// unwanted 2px-thick notch into what should be a single-pixel-wide diagonal. Because each
// Bresenham step moves by exactly one pixel in at least one axis, the result is always a
// minimal, connected, single-pixel-wide staircase with no doubled-up pixels and no gaps.
function stampHardLine(ctx, fromX, fromY, toX, toY) {
...
function stampAlongPath(ctx, fromX, fromY, toX, toY, fromRadius, toRadius, spacingPct, distanceAcc, blur = 0) {
  ...
  // stampBinaryDot always rasterizes to a single 1x1 fillRect for any radius below 1 — for
  // dy=±1, `dy*dy > r2` is already true whenever radius < 1, so every row but the center one
  // gets skipped regardless of exactly how far under 1 the radius is (0.5, 0.9, whatever).
  // That means every stamp in this segment lands as a single hard pixel no matter where along
  // the path it's sampled, so route the whole segment through the Bresenham walk above instead
  // of spaced circle sampling — that's the case that produces the doubled-pixel artifact on
  // diagonals, from two nearby samples along the path rounding independently to the same
  // column but different rows (or vice versa). This matters in practice more often than the
  // "1 canvas pixel" case alone: brush size is specified in on-screen pixels and scaled by
  // the current canvas zoom (see getCanvasPixelScale in app.js), so a "1px" brush routinely
  // ends up with a canvas-space radius somewhere under 1 rather than exactly 0.5. Segments
  // where the radius crosses the 1px threshold mid-stroke (rare — only happens when
  // pressure-driven size sweeps past it) fall through to the general path below instead; the
  // one-frame difference isn't visible.
  if (maxRadius < 1 && blur < 0.5) {
    stampHardLine(ctx, fromX, fromY, toX, toY);
    return { distanceAcc: 0, count: 1 };
  }
